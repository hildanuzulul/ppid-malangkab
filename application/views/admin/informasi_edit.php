...
<div class="main-content" style="color:#545454;">
	<div class="container-fluid py-4 mx-3 pe-5">
		<a href="<?= base_url('admin_informasi') ?>" class="btn btn-secondary text-white mb-3">
			<i class="fas fa-arrow-left"></i> Kembali
		</a>
		<h3 class="mb-4 fw-bolder">Edit Informasi</h3>

		<form action="<?= base_url('admin_informasi/update/' . $informasi->id) ?>" method="post" enctype="multipart/form-data">
			<div class="row">
				<div class="col-6">
					<div class="mb-3 text-start">
						<label for="judul" class="form-label">Judul</label>
						<input type="text" id="judul" name="judul" class="form-control" placeholder="Judul informasi"
							value="<?= htmlspecialchars($informasi->judul) ?>" required>
					...
					<div class="mb-3 text-start">
						...
						<input type="text" id="pejabat" name="pejabat" class="form-control" placeholder="Nama pejabat"
							value="<?= htmlspecialchars($informasi->pejabat) ?>" required>
					...
					<div class="mb-3 text-start">
						...
						<input type="text" id="penanggung_jawab" name="penanggung_jawab" class="form-control" placeholder="Nama penanggung jawab"
							value="<?= htmlspecialchars($informasi->penanggung_jawab) ?>" required>
					...
					<div class="mb-3 text-start">
						...
						<input type="date" id="waktu_penerbitan" name="waktu_penerbitan" class="form-control"
							value="<?= htmlspecialchars($informasi->waktu_penerbitan) ?>">
						<?php echo form_error('waktu_penerbitan', '<small class="text-danger ps-2">', '</small>'); ?>
					</div>
					<div class="mb-3 text-start">
						<label for="bentuk" class="form-label">Kategori</label>
						<select id="bentuk" name="bentuk" class="form-control form-select" placeholder="Pilih Kategori">
							...
							<option value="Soft Copy" <?= ($informasi->bentuk == 'Soft Copy') ? 'selected' : '' ?>>Soft Copy</option>
							<option value="Hard Copy" <?= ($informasi->bentuk == 'Hard Copy') ? 'selected' : '' ?>>Hard Copy</option>
							<option value="Soft dan Hard Copy" <?= ($informasi->bentuk == 'Soft dan Hard Copy') ? 'selected' : '' ?>>Soft dan Hard Copy</option>
						</select>
					...
					<div class="mb-3 text-start">
						...
						<select id="kategori" name="kategori" class="form-control form-select" placeholder="Pilih Kategori">
							...
							<option value="dikecualikan" <?= ($informasi->kategori == 'dikecualikan') ? 'selected' : '' ?>>Dikecualikan</option>
							<option value="setiap_saat" <?= ($informasi->kategori == 'setiap_saat') ? 'selected' : '' ?>>Setiap Saat</option>
							<option value="berkala" <?= ($informasi->kategori == 'berkala') ? 'selected' : '' ?>>Berkala</option>
						</select>
						<?php echo form_error('bentuk', '<small class="text-danger ps-2">', '</small>'); ?>
					</div>
				</div>
				<div class="col-6">
					<div class="mb-3 text-start">
						...
						<input type="text" id="jangka_waktu" name="jangka_waktu" class="form-control" placeholder="Contoh: 1 minggu, 1 bulan, Permanen"
							value="<?= htmlspecialchars($informasi->jangka_waktu) ?>" required>
					...
					<div class="mb-3 text-start">
						...
						<input type="text" id="media" name="media" class="form-control" placeholder="URL media informasi"
							value="<?= htmlspecialchars($informasi->media) ?>">
					...
					<div class="mb-3 text-start">
						...
						<input type="text" id="berkas" name="berkas" class="form-control" placeholder="Berkas informasi (URL file)"
							value="<?= htmlspecialchars($informasi->berkas) ?>">
					...
					<div class="mb-3 text-start">
						...
						<textarea id="ringkasan" name="ringkasan" class="form-control" rows="3" placeholder="Ringkasan isi informasi" required><?= htmlspecialchars($informasi->ringkasan) ?></textarea>
						<?php echo form_error('ringkasan', '<small class="text-danger ps-2">', '</small>'); ?>
					</div>
				</div>
			</div>
			<div class="d-flex justify-content-end">
				<a href="<?= base_url('admin_informasi') ?>" class="btn btn-secondary me-2">Batal</a>
